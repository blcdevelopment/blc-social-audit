...
| **Instagram** | ⚠️ Business Discovery (public metrics for *business* accounts, no target OAuth) | ✅ full analytics for own clients | ✅ for personal accounts / deeper post data | **Business Discovery first, scraper fallback.** Covers most builder/remodeler accounts (they're business accounts). |
...
**Build a provider-adapter with one interface and three backends, and ship them in
this order:**

1. **YouTube Data API** — prove the social pipeline end-to-end on a free, reliable
   source. (Workstream B, first.)
2. **Instagram Business Discovery** — the highest-value, compliant, free path for
   BLC's #1 niche platform; no prospect action required.
3. **One paid provider behind the adapter** (recommend **Bright Data** — $0.75/1K
   results, pay-per-success, ~98% IG success rate, and one API spanning IG/FB/
   LinkedIn/TikTok; **Apify** is the cheaper-to-start alternative at a $29/mo +
   per-result model). Use it as the **fallback** for: personal IG accounts, deeper
   post-level data, Facebook page posts, and (only if BLC asks) TikTok.
4. **Owner-OAuth** — add later, *only* for BLC's onboarded clients who want the
   richer first-party analytics (saves, reach, demographics) a public scrape can't see.

This hybrid gets BLC competitor/prospect audits (the use case OAuth can't serve)
while keeping cost and legal risk low, and isolates the one volatile dependency
(the scraper) behind a single swappable adapter.

> **Bottom line on "OAuth or scrapers like Apify?"** For an *internal prospecting/
> sales* tool, **scrapers + Instagram Business Discovery**, not OAuth. OAuth only
> audits people who opt in, which defeats the point of auditing a prospect you're
> trying to win. Keep OAuth as an optional bonus for existing clients.
...
2. **Collect** — per-platform collectors behind one provider-adapter (§3.2):
   - **YouTube** → YouTube Data API (channel stats, uploads, views, subs, engagement).
   - **Instagram** → **Business Discovery** (Graph API + FB Login) for public business-account metrics first; **scraper fallback** for personal accounts / deeper post data.
   - **Facebook / LinkedIn / (TikTok)** → scraper fallback, or owner-OAuth for BLC's own clients. **LinkedIn deferred / lowest priority** (enforcement risk, §3.2.2).
...
- P2-B2 Instagram via Business Discovery (free, public business metrics)
- P2-B2b Scraper fallback backend (Bright Data/Apify) for FB / personal IG / deeper data; LinkedIn/TikTok optional
...
FastAPI API (+ team auth)  ──►  Celery workers + Redis
 ...
 PostgreSQL  crawler/PSI/CrUX     YouTube API · IG Business           SEMrush/Ahrefs,
 (managed)   SEO/UX/schema/a11y   Discovery · scraper adapter         GA4/GSC/Clarity
        |        \                   /        \                          /
        |         v                 v          v                        v
...
| Instagram (public) | **Instagram Business Discovery** (Graph API + FB Login) | Free; public metrics for business accounts, no target OAuth (§3.2) |
| IG/FB/LinkedIn/TikTok (fallback) | **Bright Data** ($0.75/1K, pay-per-success) or **Apify** ($29/mo + per-result) | Behind one provider adapter; for personal/competitor/deeper data |
...
- **Wk 0 (2–3 days):** Phase 2.0 — confirm social path (recommend YouTube + IG Business Discovery + one scraper fallback) + budget + legal sign-off; draft `social.yaml`; choose hosting/auth/storage.
...
- **Wk 4:** B — **Instagram Business Discovery** backend + scraper fallback (per §3.2); social extractors + fixtures.
...
tiers; the only new variable cost is the scraping fallback, and it is small.
...
|---|---|---|
| **Social data access** (Technical Assessment §2.1) | Could block IG/FB/LinkedIn | Hybrid §3.2: YouTube + IG Business Discovery (free) cover most of the niche; scraper only as fallback, behind one adapter |
...
| IG Business Discovery field limits | Missing personal-account / deep data | Documented fallback to scraper for those cases; set audit expectations per account type |
...
- **B:** Submitting website + social handles produces a deterministic **Social Score**
  per platform **without requiring the audited account to log in** (YouTube API + IG
  Business Discovery, scraper fallback), grounded social commentary with tiered
...
1. **§3.2 confirmed** — sign off the recommended hybrid (YouTube + IG Business
   Discovery + one paid scraper fallback) and pick the provider (Bright Data vs Apify)
   and a monthly budget cap. (§3.1 internal-tool is already locked.)
2. **Accounts & keys for the chosen path:**
   ...
   - A **Bright Data** (or Apify) account if/when the scraper fallback is enabled.
